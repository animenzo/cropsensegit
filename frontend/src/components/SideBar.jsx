    ...
    <>
      ...
      `}>
        ...
        <div className="flex items-center justify-between h-20 px-8 border-b border-gray-100">
          ...
          <button onClick={toggleSidebar} className="lg:hidden text-gray-500 hover:text-emerald-600">
            <FaTimes size={24} />
          </button>
        </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                `}
              >
                <item.icon className={`text-xl ${isActive ? 'text-emerald-600' : 'text-gray-400 group-hover:text-emerald-500'}`} />
                <span>{item.name}</span>
        ...
        <div className="absolute bottom-0 w-full p-4 border-t border-gray-100 bg-gray-50/50">
          <div className="flex items-center gap-3 mb-4 px-2">
            ...
            <div className="overflow-hidden">
              <p className="text-sm font-bold text-gray-700 truncate">{user?.name || 'Farmer'}</p>
